...
run_sdy2583_script_sequence <- function(panel, scripts, validate_script=NULL, stop_on_missing=TRUE) {
  ...
  Sys.setenv(SDY2583_REPO_ROOT=root,SDY2583_CURRENT_PANEL=toupper(panel))
  ...
  status_dir<-file.path(root,"outputs",toupper(panel),"validation");dir.create(status_dir,recursive=TRUE,showWarnings=FALSE)
  utils::write.csv(status,file.path(status_dir,paste0("SDY2583_",toupper(panel),"_pipeline_execution_status.csv")),row.names=FALSE)
